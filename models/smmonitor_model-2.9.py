...
class SmmonitorProjectTask(models.Model):
    ...
    def default_get(self, fields):
        ...
        if 'user_timezone_offset' in fields:
            tz_offset = self.env['ir.config_parameter'].sudo().get_param('tz_offset')
            if tz_offset:
                res['user_timezone_offset'] = int(tz_offset)
        return res
    
    @api.model
    def create(self, vals): #Metodo para asegurar que el "user_timezone_offset" se establezca al crear nuevas tareas.
        if 'user_timezone_offset' not in vals:
            tz_offset = self.env['ir.config_parameter'].sudo().get_param('tz_offset')
            if tz_offset:
                vals['user_timezone_offset'] = int(tz_offset)
        return super(SmmonitorProjectTask, self).create(vals)
    ...
    def _check_temp_values(self):
        for task in self:
            # Obtener el desplazamiento de la cookie
            tz_offset = task.user_timezone_offset
    ...
    """ class SmmonitorAnalyticsGraph(models.Model):
    _name = 'project.task.analyticsgraph.smmonitor'
    _description = 'Gráficas análiticas personalizadas para proyectos de RR.SS.'

    @api.model
    def get_filtered_data(self, start_date, end_date):
        domain = [
            ('datetakesdata', '>=', start_date),
            ('datetakesdata', '<=', end_date)
        ]
        records = self.search(domain)

        return {
            'labels': records.mapped('datetakesdata'),
            'engagement': records.mapped('engagement'),
            'interactions': records.mapped('interactions'),
            'reach': records.mapped('reach'),
            'impressions': records.mapped('impressions'),
        } """