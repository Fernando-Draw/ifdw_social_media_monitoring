...
class SmmonitorData(http.Controller):
    @http.route('/smmonitor/get_filtered_data', type='json', auth='user')
    def get_filtered_data(self, start_date, end_date):
        task_ids = request.env['project.task.smmonitor'].search([
            ('datetakesdata', '>=', start_date),
            ('datetakesdata', '<=', end_date)
        ])

        labels = [task.datetakesdata for task in task_ids]
        engagement = [task.engagement for task in task_ids]
        interactions = [task.interactions for task in task_ids]
        reach = [task.reach for task in task_ids]
        impressions = [task.impressions for task in task_ids]
